...
import { calculateNetSalary } from 'maroctax';
...
export class MaroctaxPayslipComponent {
  /** Monthly gross salary in MAD. */
  readonly gross = input.required<number>();

  /** Number of dependents (0–6). */
  readonly dependents = input(0);

  readonly breakdown = computed(() =>
    calculateNetSalary(this.gross(), this.dependents()),
  );
}
